...
void main() {
    ...
    #ifndef Chams_prioritizedamagetint
        col.rgb = mix(col.rgb, entityColor.rgb, entityColor.a);
    #endif

    bool highlight = true; // global highlight flag

    #ifdef Chams_prioritizedamagetint
        if (entityColor.a > 0.0) {
            highlight = false; // disable highlight if entityColor is set
        }
    #endif
    ...
    if (highlight && (entityId != 1100 || Chams_targets == 1)) {
